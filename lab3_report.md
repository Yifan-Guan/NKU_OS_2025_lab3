...
![qemu_output](./lab3_report.assets/qemu_output.png)
...
- **对应的OS原理知识点**：中断/异常机制
- 实验中的知识点是OS原理在RISC-V架构上的具体实现。`stvec`、`sepc`、`scause`等CSR是原理中"保存现场"和"控制权转移"的具体承载者。
- 原理更抽象，强调"中断"和"异常"的区别（异步/同步）；实验更具体，关注的是特定寄存器的操作。

### 1.2 上下文保存与恢复

- **对应的OS原理知识点**：中断/异常机制
- 实验中指在__alltraps中，通过SAVE_ALL宏将所有通用寄存器和sstatus、sepc等CSR保存到内核栈上的trapframe结构体中，并在返回前通过RESTORE_ALL和sret恢复。OS原理中指保存被中断程序的执行状态（寄存器、PC、PSW等），以便在处理完成后能透明地恢复执行。

### 1.3 时钟中断处理

- **对应的OS原理知识点**：时钟中断

- 实验中指响应IRQ_S_TIMER中断，通过clock_set_next_event()设置下一次中断，并维护一个ticks计数器。OS原理中指OS依赖的周期性"心跳"，用于驱动进程调度、时间片轮转、计时器等。
- 实验实现了OS原理中最底层的时钟中断处理。ticks计数器是OS实现所有计时功能（如uptime、sleep）的基础。

### 1.4 非法指令与断点异常处理

- **对应的OS原理知识点**：异常（陷阱）处理/系统调用机制
- ebreak（断点）指令是实现"系统调用"（System Call）和调试器的基础。OS原理中的"系统调用"就是一种由程序主动触发的"陷阱"（Trap）。实验通过处理ebreak，触及了系统调用机制的入口。

## 2.OS原理中重要但在实验中未对应的知识点

### 2.1 进程上下文切换

- **原理**：指OS暂停一个正在运行的进程（或内核线程），保存其完整的执行上下文，然后加载另一个进程的上下文并恢复执行。

### 2.2 进程调度

- **原理**：OS的核心功能之一。时钟中断处理程序（ISR）在完成ticks计数后，通常会调用调度器（scheduler），调度器根据调度算法（如FIFO、RR、MLFQ）决定是否要进行进程切换，以及下一个应该运行哪个进程。
